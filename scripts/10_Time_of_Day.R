...
#Description:       10_Time_of_Day.R estimates a start time for each tour
#Date:              June 30, 2014
...
#Step 10 Time of Day
#-----------------------------------------------------------------------------------
progressStart(tourtod,3)

#-----------------------------------------------------------------------------------
#Apply Time of Day Model
#-----------------------------------------------------------------------------------
progressNextStep("Apply Time of Day Model")

## # -- Heither, 12-22-2015: R wouldn't recognize tod (from tourtod) on Line 35 so I added the next line (NOTE: stringsAsFactors=FALSE is necessary for this to work)
tod <- read.csv(file=paste0(model$inputdir,"/model_timeofday.csv"),header=T,sep=",",stringsAsFactors=FALSE)
...
tourtod$summaryfunc <- function(steplist){
  ...
  tourtodlist <- list()
  tourtodlist$todbytourtype <- dcast.data.table(todbytourtype,TODCat+TOD~TourType,fun=sum,value.var="N")[,list(TOD,Direct_Tours,Peddling_Tours)]            
  return(tourtodlist)
  
}

tourtod <- progressEnd(tourtod)
